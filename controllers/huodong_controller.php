...
class HuodongController extends AppController {
	...
	function index(){
		...
		//如果为推广来源，抽奖10个集分宝
		if(isset($_GET['code']) && $_GET['code'] && ($_GET['code'] == md5(date('Ymd').'kkey') || $_GET['code'] == md5(date('Ymd', time()-DAY).'kkey') || $_GET['code'] == md5(date('Ymd', time()-2*DAY).'kkey')) && !D('myuser')->islogined()){
			$prize = 3;
		}else{
			$prize = 4;
		}
